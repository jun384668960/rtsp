 ...
 * @成功则返回1 , 失败则返回0
 */
int h264_decode_sps(BYTE * buf, unsigned int nLen, int *width, int *height, double *fps)
{
	UINT StartBit = 0;
	*fps = 0;
		...
		*height = (pic_height_in_map_units_minus1 + 1) * 16;
		...
		if (vui_parameter_present_flag)
		{
			...
			if (timing_info_present_flag)
			{
				...
				printf("time_scale:%d num_units_in_tick:%d\n",time_scale,num_units_in_tick);
				*fps = (double)time_scale / (double)(2 * num_units_in_tick);
			}
		}
		return 1;
	}
