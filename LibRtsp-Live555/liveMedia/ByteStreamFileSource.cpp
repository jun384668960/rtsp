...
void ByteStreamFileSource::seekToByteAbsolute(u_int64_t byteNumber, u_int64_t numBytesToStream) {
  ...
  fprintf(stderr,"seekToByteAbsolute()\n");
...
void ByteStreamFileSource::seekToByteRelative(int64_t offset, u_int64_t numBytesToStream) {
  SeekFile64(fFid, offset, SEEK_CUR);
	fprintf(stderr,"seekToByteRelative()\n");
  fNumBytesToStream = numBytesToStream;
  fLimitNumBytesToStream = fNumBytesToStream > 0;
}
...
ByteStreamFileSource::~ByteStreamFileSource() {
  ...
  CloseInputFile(fFid);
	fprintf(stderr,"~ByteStreamFileSource()\n");

}

void ByteStreamFileSource::doGetNextFrame() {
	fprintf(stderr,"===>>>doGetNextFrame()\n");
...
void ByteStreamFileSource::doStopGettingFrames() {
	fprintf(stderr,"doStopGettingFrames\n");
...
void ByteStreamFileSource::fileReadableHandler(ByteStreamFileSource* source, int /*mask*/) {
  if (!source->isCurrentlyAwaitingData()) {
  	fprintf(stderr,"fileReadableHandler\n");
    source->doStopGettingFrames(); // we're not ready for the data yet
    return;
  }
  fprintf(stderr,"doReadFromFile\n");
...
void ByteStreamFileSource::doReadFromFile() {
  ...
  fprintf(stderr,"doReadFromFile doReadFromFile :%d %p\n",fMaxSize, fFid);
#ifdef READ_FROM_FILES_SYNCHRONOUSLY
  fFrameSize = fread(fTo, 1, fMaxSize, fFid);
#else
	fprintf(stderr,"doReadFromFile doReadFromFile fFidIsSeekable:%d\n", fFidIsSeekable);
  if (fFidIsSeekable) {
    fFrameSize = fread(fTo, 1, fMaxSize, fFid);
  } else {
...
#endif
  if (fFrameSize == 0) {
  	fprintf(stderr,"doReadFromFile doReadFromFile\n");
    handleClosure();
    return;
  }
