...
#define LIVE_STREAM_RUL "LiveStream"
...
::lookupServerMediaSession(char const* streamName, Boolean isFirstLookupInSession) {
  ...
  if (strcmp(streamName, LIVE_STREAM_RUL) == 0)
  {
     fileExists = 1;
  }
...
static ServerMediaSession* createNewSMS(UsageEnvironment& env,
  ...
  if (extension == NULL && strcmp(fileName, LIVE_STREAM_RUL)) return NULL;

  ServerMediaSession* sms = NULL;
  Boolean const reuseSource = False;
  if(strcmp(fileName, LIVE_STREAM_RUL) == 0){
		// Assumed to be a H.265 Live Stream
		debug_log(LEVEL_DEBUG,"Assumed to be a H.265 Live Stream");
		NEW_SMS(LIVE_STREAM_RUL" Video");
		//OutPacketBuffer::maxSize = 200 * 1024;//100000; // allow for some possibly large frames
        LiveStreamMediaSource* InputDevice = LiveStreamMediaSource::createNew(env);
		OutPacketBuffer::maxSize = InputDevice->videoSource()->maxFrameSize();
		
		sms->addSubsession(H264VideoLiveServerMediaSubsession::createNew(env, *InputDevice, reuseSource));
		//sms->addSubsession(H265VideoLiveServerMediaSubsession::createNew(env, *InputDevice, reuseSource));
		sms->addSubsession(ADTSAudioLiveServerMediaSubsession::createNew(env, *InputDevice, reuseSource));
