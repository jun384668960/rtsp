...
void socketkeepalive(int sockfd, int alive, int idle, int interval, int count)
{
    int keepAlive=alive;//开启keepalive属性
    int keepIdle=idle;//如该连接在5秒内没有任何数据往来，则进行探测
    int keepInterval=interval;//探测时发包的时间间隔为2秒
    int keepCount=count;//探测尝试的次数。如果第1次探测包就收到响应了，则后2次的不再发送
