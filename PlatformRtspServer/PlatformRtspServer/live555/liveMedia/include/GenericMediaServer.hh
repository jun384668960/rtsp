...
#include "TlsSocketHelper.hh" //support tls1.2
...
public:
  ...
  virtual ServerMediaSession* lookupServerMediaSession(char const* streamName, void* pClientConn, Boolean isFirstLookupInSession = True);
...
protected:
  GenericMediaServer(UsageEnvironment& env, int ourSocket, Port ourPort,
		     unsigned reclamationSeconds, bool supportTls12 = false);
...
public:
	//add by 2018/04/24
	void AddSupportTls(int serverSocket, Boolean supportTls);
	virtual Boolean isSupportTls(int serverSocket) const;
#define MAX_OPEN_PORT 5
	int fSvrSockets[MAX_OPEN_PORT];
	Boolean fSupportTls[MAX_OPEN_PORT];
	int fCurOpenPortCount;

public: // should be protected, but some old compilers complain otherwise
  // The state of a TCP connection used by a client:
  class ClientConnection {
  protected:
    ClientConnection(GenericMediaServer& ourServer, int clientSocket, struct sockaddr_in clientAddr,Boolean supportTls = False);
	...
	virtual Boolean isSupportTls() const { return fSupportTls; } 
...
protected:
  virtual ClientConnection* createNewClientConnection(int clientSocket, struct sockaddr_in clientAddr, int serverSocket = 0) = 0;
