...
::RTSPServerSupportingHTTPStreaming(UsageEnvironment& env, int ourSocket, Port rtspPort,
  ...
  : RTSPServer(env, ourSocket, rtspPort, authDatabase, reclamationTestSeconds,supportTls12) {
}
...
RTSPServerSupportingHTTPStreaming::createNewClientConnection(int clientSocket, struct sockaddr_in clientAddr, int serverSocket) {
	//printf("create new client connection by http,portnum = %d,\n",httpServerPortNum());
  return new RTSPClientConnectionSupportingHTTPStreaming(*this, clientSocket, clientAddr,isSupportTls(serverSocket));
}

RTSPServerSupportingHTTPStreaming::RTSPClientConnectionSupportingHTTPStreaming
::RTSPClientConnectionSupportingHTTPStreaming(RTSPServer& ourServer, int clientSocket, struct sockaddr_in clientAddr,Boolean supportTls)
  : RTSPClientConnection(ourServer, clientSocket, clientAddr,supportTls),
...
::handleHTTPCmd_StreamingGET(char const* urlSuffix, char const* /*fullRequestStr*/) {
  ...
  do {
	...
	break;
      ...
      //printf("RTSPServerSupportingHTTPStreaming::RTSPClientConnectionSupportingHTTPStreaming::handleHTTPCmd_StreamingGET: sock = %d,cmd = %s\n",fClientOutputSocket,(char*)fResponseBuffer);
  ...
  send(fClientOutputSocket, (char const*)fResponseBuffer, strlen((char*)fResponseBuffer), 0);
  //printf("111 RTSPServerSupportingHTTPStreaming::RTSPClientConnectionSupportingHTTPStreaming::handleHTTPCmd_StreamingGET: sock = %d,cmd = %s\n",fClientOutputSocket,(char*)fResponseBuffer);
  fResponseBuffer[0] = '\0'; // We've already sent the response.  This tells the calling code not to send it again.
