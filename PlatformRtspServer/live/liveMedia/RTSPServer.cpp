...
Boolean RTSPServer::setUpTunnelingOverHTTP(Port httpPort) {
  ...
  if (fHTTPServerSocket >= 0) {
	...
	//printf("RTSPServer::setUpTunnelingOverHTTP dst port = %d\n",httpServerPortNum());
	AddSupportTls(fHTTPServerSocket,httpServerPortNum() == 443);
...
RTSPServer::RTSPServer(UsageEnvironment& env,
		       ...
		       unsigned reclamationSeconds,
			   bool supportTls12)
  : GenericMediaServer(env, ourSocket, ourPort, reclamationSeconds,supportTls12),
...
::RTSPClientConnection(RTSPServer& ourServer, int clientSocket, struct sockaddr_in clientAddr,Boolean supportTls)
  : GenericMediaServer::ClientConnection(ourServer, clientSocket, clientAddr,supportTls),
    fOurRTSPServer(ourServer), fClientInputSocket(fOurSocket), fClientOutputSocket(fOurSocket),
    fIsActive(True), fRecursionCount(0), fOurSessionCookie(NULL), fGssLiveConn(NULL) {
  resetRequestBuffer();
}
...
::handleCmd_DESCRIBE(char const* urlPreSuffix, char const* urlSuffix, char const* fullRequestStr) {
	//printf("=========RTSPServer::RTSPClientConnection::handleCmd_DESCRIBE start\n ");
  ...
  do {
	...
	//printf("urlTotalSuffix = %s,fullRequeststr = %s\n",urlTotalSuffix,fullRequestStr); 
    if (!authenticationOK("DESCRIBE", urlTotalSuffix, fullRequestStr)) 
	{
		LOG_ERROR("[DESCRIBE]  authentication failed!");
		break;
	}
...
//			LOG_INFO("~RTSPClientConnection fGssLiveConn:%p referenceCount:%d", fGssLiveConn, fGssLiveConn->referenceCount());
    ...
    if (sdpDescription == NULL) {
	  ...
	  LOG_ERROR("[DESCRIBE] session generateSDPDescription failed!");
      break;
    }
  ...
  if (session != NULL) {
    ...
    if (session->referenceCount() == 0 && session->deleteWhenUnreferenced()) {
	  ...
	  //printf("describe fOurServer.removeServerMediaSession(session);");
	  //fOurServer.removeServerLiveConns(fStreamName);
    }
  }

  delete[] sdpDescription;
  delete[] rtspURL;
  LOG_INFO("[DESCRIBE] success: fullRequeststr = %s,client->%s:%d",fullRequestStr,inet_ntoa(fClientAddr.sin_addr),ntohs(fClientAddr.sin_port)); 
  LOG_INFO("[DESCRIBE] response: %s\n",(char*)fResponseBuffer);
  //printf("=========RTSPServer::RTSPClientConnection::handleCmd_DESCRIBE end\n ");
}
...
void RTSPServer::RTSPClientConnection::handleRequestBytes(int newBytesRead) {
  ...
  do {
	...
	{
		FdStus fdstus;
		TlsHelper_WriteNonBlock(fTlsSocket,fResponseBuffer, strlen((char*)fResponseBuffer), &fdstus);
		//printf("**********************tls write buffer: \n%s\n\n",(char*)fResponseBuffer);
	}
	else
	{
		send(fClientOutputSocket, (char const*)fResponseBuffer, strlen((char*)fResponseBuffer), 0);
		//printf("write buffer,fClientOutputSocket = %d: \n%s\n\n",fClientOutputSocket,(char*)fResponseBuffer);
	}
...
::handleCmd_SETUP(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  //printf("=========RTSPServer::RTSPClientConnection::handleCmd_SETUP start\n ");
  ...
  do {
    ...
    ServerMediaSession* sms = NULL;
 	if(strstr(streamName,"uid=") != NULL)
 		sms = fOurServer.lookupServerMediaSession(streamName,ourClientConnection, fOurServerMediaSession == NULL);
 	else
      sms = fOurServer.lookupServerMediaSession(streamName, fOurServerMediaSession == NULL);
    //printf("handleCmd_SETUP look up sms = %p\n",sms);
    if (sms == NULL) {
	...
	LOG_ERROR("[SETUP] can't find mediasession");
      } else {
	...
	LOG_ERROR("[SETUP] The client asked for a stream that doesn't exist, but using a stream id for a stream that does exist. Bad request");
      }
      break;
    } else {
      if (fOurServerMediaSession == NULL) {
	...
	ourClientConnection->handleCmd_bad();
	LOG_ERROR("[SETUP] The client asked for a stream that's different from the one originally requested for this stream id.  Bad request:");
	break;
      }
    }
	...
	fStreamStates[i].tlsSocket = TlsHelperObjNull;
      }
    }
	...
	ourClientConnection->handleCmd_notFound();
	LOG_ERROR("[SETUP] The specified track id doesn't exist, so this request fails: trackId = %s",trackId);
	break;
      }
    } else {
	...
	ourClientConnection->handleCmd_bad();
	LOG_ERROR("[SETUP] Weird case: there was no track id in the URL. This works only if we have only one subsession:");
	break;
      }
      trackNum = 0;
      subsession = fStreamStates[trackNum].subsession;
    ...
    if (streamingMode == RTP_TCP) {
	  ...
	  fStreamStates[trackNum].tlsSocket = ourClientConnection->fTlsSocket;
    ...
    subsession->getStreamParameters(fOurSessionId, ourClientConnection->fClientAddr.sin_addr.s_addr,
					...
					fStreamStates[trackNum].tlsSocket);
...
::handleCmd_PLAY(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  //printf("=========RTSPServer::RTSPClientConnection::handleCmd_PLAY start\n ");
  char* rtspURL
  ...
  LOG_INFO("[PLAY] fResponseBuffer = %s",(char*)ourClientConnection->fResponseBuffer);
  LOG_INFO("[PLAY] response: %s\n",(char*)ourClientConnection->fResponseBuffer);
  //printf("=========RTSPServer::RTSPClientConnection::handleCmd_PLAY end\n ");
}
...
RTSPServer::createNewClientConnection(int clientSocket, struct sockaddr_in clientAddr, int serverSocket) {
  return new RTSPClientConnection(*this, clientSocket, clientAddr,isSupportTls(serverSocket));
}
