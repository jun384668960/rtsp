...
::getStreamParameters(unsigned clientSessionId,
  ...
  if (tcpSocketNum < 0) { // UDP
    ...
    destinations = new Destinations(tcpSocketNum, rtpChannelId, rtcpChannelId,tlsSock);
  }
...
::startPlaying(Destinations* dests, unsigned clientSessionId,
  ...
  if (fRTCPInstance == NULL && fRTPSink != NULL) {
	...
	//传不进去，rtp interface的fowner == env()
// 	fRTCPInstance->SetIsSupportTls((Boolean)(dests->tlsSocket != TlsHelperObjNull));
// 	printf("create rtcp fRTCPInstance = %p, support tls = %d\n",fRTCPInstance,(Boolean)(dests->tlsSocket != TlsHelperObjNull));
  ...
  if (dests->isTCP) {
    ...
    if (fRTPSink != NULL) {
      fRTPSink->addStreamSocket(dests->tcpSocketNum, dests->rtpChannelId,dests->tlsSocket);
      RTPInterface
    ...
    if (fRTCPInstance != NULL) {
      fRTCPInstance->addStreamSocket(dests->tcpSocketNum, dests->rtcpChannelId, dests->tlsSocket);
