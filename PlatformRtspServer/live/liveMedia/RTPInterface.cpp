...
public:
  SocketDescriptor(UsageEnvironment& env, int socketNum,TlsHelperObj tlsSock = TlsHelperObjNull);
...
static SocketDescriptor* lookupSocketDescriptor(UsageEnvironment& env, int sockNum, Boolean createIfNotFound = True, TlsHelperObj tlsSock = TlsHelperObjNull) {
  ...
  if (socketDescriptor == NULL) {
    if (createIfNotFound) {
      socketDescriptor = new SocketDescriptor(env, sockNum, tlsSock);
...
RTPInterface::RTPInterface(Medium* owner, Groupsock* gs)
  : fOwner(owner), fGS(gs),
    fTCPStreams(NULL),
    fNextTCPReadSize(0), fNextTCPReadStreamSocketNum(-1), fNextTcpTlsReadStreamSocketNum(TlsHelperObjNull),
...
void RTPInterface::addStreamSocket(int sockNum,
  ...
  fTCPStreams = new tcpStreamRecord(sockNum, streamChannelId, fTCPStreams, tlsSock);

  // Also, make sure this new socket is set up for receiving RTP/RTCP-over-TCP:
  //printf("RTPInterface::addStreamSocket socknum = %d, tls = %p\n",sockNum,tlsSock);
  SocketDescriptor* socketDescriptor = lookupSocketDescriptor(envir(), sockNum, True, tlsSock);
...
Boolean RTPInterface::sendPacket(unsigned char* packet, unsigned packetSize) {
  ...
  for (tcpStreamRecord* stream = fTCPStreams; stream != NULL; stream = nextStream) {
    ...
    if (!sendRTPorRTCPPacketOverTCP(packet, packetSize,
				    stream->fStreamSocketNum, stream->fStreamChannelId,stream->fTlsSocket)) {
      success = False;
    }
  }
...
// printf("RTPInterface::startNetworkReading socknum = %d, tls = %p\n",streams->fStreamSocketNum,streams->fTlsSocket);
    SocketDescriptor* socketDescriptor = lookupSocketDescriptor(envir(), streams->fStreamSocketNum, True, streams->fTlsSocket);
...
Boolean RTPInterface::handleRead(unsigned char* buffer, unsigned bufferMaxSize,
  ...
  if (fNextTCPReadStreamSocketNum < 0) {
    ...
    int curBytesRead;
	if ( fNextTcpTlsReadStreamSocketNum )
	{
		while (1) 
		{
			FdStus fdstus;
			curBytesRead = TlsHelper_ReadNonBlock(fNextTcpTlsReadStreamSocketNum, &buffer[bytesRead], curBytesToRead, &fdstus);
			if( curBytesRead != curBytesToRead && fdstus != FDSTUS_WANT_READ)
			{
				//printf("RTPInterface::handleRead Tls read failed!XXXXXXXXXXXXXXX\n");
				LOG_ERROR("[RTPINTERFACE] TlsHelper_ReadNonBlock fdstus = %d, curBytesRead = %d",fdstus,curBytesRead);
				curBytesRead = -1; //异常
				break;
			}
			else if (curBytesRead != curBytesToRead && fdstus == FDSTUS_WANT_READ)
			{
				bytesRead += curBytesRead; //读完了，等待继续读取
				break;
			}
	...
	else
	{
		...
		//printf("RTPInterface::handleRead socket = %d,%s\n",fNextTCPReadStreamSocketNum,&buffer[0]);
	}
...
Boolean RTPInterface::sendRTPorRTCPPacketOverTCP(u_int8_t* packet, unsigned packetSize,
						 int socketNum, unsigned char streamChannelId, TlsHelperObj tlsSock) {
#ifdef DEBUG_SEND
  ...
  do {
    ...
    if (!sendDataOverTCP(socketNum, framingHeader, 4, False, tlsSock)) break;

    if (!sendDataOverTCP(socketNum, packet, packetSize, True, tlsSock)) break;
#ifdef DEBUG_SEND
...
Boolean RTPInterface::sendDataOverTCP(int socketNum, u_int8_t const* data, unsigned dataSize, Boolean forceSendToSucceed, TlsHelperObj tlsSock) {
  int sendResult = 0;
  //printf("RTPInterface::sendDataOverTCP ,is supporttls = %d, fOwner = %p\n",fOwner->isSupportTls(),fOwner);
  if(tlsSock)
  {
	  FdStus fdstus;
	  sendResult = TlsHelper_WriteNonBlock(tlsSock,(char*)data, dataSize,&fdstus);
	  //printf("tls send data sendResult = %d, need send size = %d,data->\n",sendResult,dataSize);
// 	  int loopc = 60 < (int)dataSize ? 60 : (int)dataSize;
...
// 	  printf("send data sendResult = %d, need send size = %d,data->\n",sendResult,dataSize);
// 	  int loopc = 60 < (int)dataSize ? 60 : (int)dataSize;
// 	  for (int i = 0; i <  loopc; i++)
// 	  {
// 		printf("%02x",*((char*)(data+i)));
...
// 	  printf("\n");
  ...
  if (sendResult < (int)dataSize) {
    ...
    if (numBytesSentSoFar > 0 || (forceSendToSucceed && envir().getErrno() == EAGAIN)) {
	  ...
	  if(tlsSock)
	  {
		  sendResult = TlsHelper_WriteBlock(tlsSock,(char *)data, dataSize);
	  }
	  else
	  {
...
SocketDescriptor::SocketDescriptor(UsageEnvironment& env, int socketNum, TlsHelperObj tlsSock)
  :fEnv(env), fOurSocketNum(socketNum), fTlsSocket(tlsSock),
...
Boolean SocketDescriptor::tcpReadHandler1(int mask) {
  ...
  if (fTCPReadingState != AWAITING_PACKET_DATA) {
	...
	{
		result = readSocket(fEnv, fOurSocketNum, &c, 1, fromAddress);
//		printf("SocketDescriptor::tcpReadHandler read, fOurSocketNum = %d result = %d, %c\n",fOurSocketNum,result,(char)c);
	}
	else
	{
		FdStus fdstus;
		result = TlsHelper_ReadNonBlock(fTlsSocket,&c,1,&fdstus);
		if( result != 1 && fdstus == FDSTUS_WANT_READ)
			result = 0;
		else if (result != 1 && fdstus != FDSTUS_WANT_READ)
		{
//			printf("SocketDescriptor::tcpReadHandler1 tls read failed, fOurSocketNum = %d!XXXXXXXXX,tlssocket = %p\n",fOurSocketNum,fTlsSocket);
			result = -1;
		}
	}
  ...
  switch (fTCPReadingState) {
	...
	rtpInterface->fNextTcpTlsReadStreamSocketNum = fTlsSocket;
	...
	if (rtpInterface->fReadHandlerProc != NULL) {
	  ...
	  {
		  result = readSocket(fEnv, fOurSocketNum, &c, 1, fromAddress);
//		  printf("SocketDescriptor::tcpReadHandler 111 read sock = %d result = %d, %c\n",fOurSocketNum,result,(char)c);
	  }
	  else
	  {
		  FdStus fdstus;
...
::tcpStreamRecord(int streamSocketNum, unsigned char streamChannelId,
		  tcpStreamRecord* next, TlsHelperObj tlsSocket)
  : fNext(next),
    fStreamSocketNum(streamSocketNum), fStreamChannelId(streamChannelId), fTlsSocket(tlsSocket) {
}
